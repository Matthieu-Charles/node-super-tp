...
// DATE
controller.getBarCommandesByDate = async (req, res) => {
	const id_Bar = req.params.id_bar;
	const date = req.query.date; // Notez que nous utilisons req.query.date pour obtenir la date
	...
	try {
			...
			// Recherche des commandes pour le bar donné et la date spécifiée
			...
			console.error("Failed fetching bar commandes:", error);
			return res.status(500).send({ message: "Failed fetching bar commandes", error });
	}
};
