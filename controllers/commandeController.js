...
controller.getById = (req, res) => {
  ...
  Commande.findByPk(id, { include: ["Bieres"] })
    .then((commande) => {
      if (!commande) {
        return res.send({ message: "Commande not found" });
      }
...
controller.update = (req, res) => {
  const id = req.params.id;
  const { name, prix, date, status } = req.body;
  const modifiedCommande = { name, prix, date, status };
    ...
    .then((commande) => {
      ...
      if (commande.status === "terminée") {
        return res.status(401).send({
          message: "Commande is already closed and can't be modified",
        });
      }
      Commande.update(modifiedCommande, { where: { id } })
        .then((commande) => {
          return res
            .status(200)
            .send({ commande, message: "Commande updated !" });
        })
        .catch((error) => {
...
controller.addBiereToCommande = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Bière ajoutée à la commande avec succès" });
  } catch (error) {
  ...
  CommandeBiere.destroy({
    ...
    .then((result) => {
      if (result === 1) {
        return res
          .status(200)
          .send({ message: "Biere removed from Commande successfully" });
      } else {
    ...
    .catch((error) => {
      ...
      res
        .status(400)
        .send({ message: "Failed removing Biere from Commande", error });
    });
};
